...
const DEFAULT_DROP_SELECTORS = [
  ...
  // Deliberately-empty media fallbacks. Real sites ship e.g.
  // <source src="" type="video/mp4"> alongside a real .webm (dropbox.com does
  // this on every product video). The browser's media-source selection
  // algorithm never picks an empty src, so dropping the node changes no pixel —
  // but React logs 'An empty string ("") was passed to the src attribute' once
  // per element, and viewport-check's console-errors gate has NO baseline path,
  // so it fails every viewport. Matches only a genuinely empty attribute (an
  // absent one reads back undefined), never a populated source.
  'source[src=""]',
  '[data-replica-drop]',
];
