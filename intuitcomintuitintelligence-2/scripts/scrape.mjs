...
async function download(url, contentTypeHint, referer) {
  ...
  // The URL alone can misclassify an asset, and the response knows better.
  //
  // A CDN endpoint like `/privacy-seal/seal?rid=...` has no file extension, and
  // markup-harvested refs carry no Content-Type hint (only network.json does),
  // so kindOf falls back to 'other' and localName gives the file NO extension.
  // The download still succeeds, so nothing errors and no warning prints -- but
  // `public/assets/other/seal.<hash>` is then served as
  // `application/octet-stream`, and a browser will not render an <img> whose
  // response carries that MIME type. The image reports naturalWidth === 0, i.e.
  // the viewport gate's "broken images" failure, for a file that is on disk and
  // returns 200 -- a confusing symptom whose cause is purely the missing
  // extension.
  //
  // The response's own Content-Type is authoritative, so reclassify once it is
  // known. Extension-bearing URLs are unaffected: kindOf/localName both prefer
  // the URL's own extension, so this only ever fills in a blank.
  let outKind = kind;
  let outName = name;
  let outRawPath = rawPath;
  let outPubPath = pubPath;

  const headerType = (res.headers.get('content-type') ?? '').split(';')[0].trim();
  // Some hosts serve SVG as octet-stream/text-xml. The bytes settle it, and an
  // <img> needs `image/svg+xml` to render at all, so sniff before trusting a
  // non-image header.
  const looksSvg = /^\s*(?:<\?xml[^>]*>\s*)?(?:<!--[\s\S]*?-->\s*)*(?:<!DOCTYPE[^>]*>\s*)*<svg[\s>]/i.test(
    buf.subarray(0, 1024).toString('latin1'),
  );
  const realType = !/^image\//i.test(headerType) && looksSvg ? 'image/svg+xml' : headerType;

  if (realType) {
    const reKind = kindOf(url, realType);
    const reName = localName(url, realType);
    if (reKind !== outKind || reName !== outName) {
      outKind = reKind;
      outName = reName;
      const reRawDir = path.join(paths.raw, reKind);
      const rePubDir = path.join(paths.publicAssets, reKind);
      await mkdir(reRawDir, { recursive: true });
      await mkdir(rePubDir, { recursive: true });
      outRawPath = path.join(reRawDir, reName);
      outPubPath = path.join(rePubDir, reName);
    }
  }

  await writeFile(outRawPath, buf);
  await writeFile(outPubPath, buf);
  return { url, kind: outKind, name: outName, bytes: buf.length, buf };
}
