...
async function findHiddenContent(page, scrollSettleMs, thorough) {
  ...
  return page.evaluate(
    async ({ quickMs: quick, confirmMs: confirm, thorough: strict }) => {
      ...
      // or visibility, where the structural walk above cannot see it. A real
      // example: a country-selector dropdown styled
      //   { opacity: 0; clip-path: inset(0 0 100% 0); pointer-events: none }
      // with a separate `show` class restoring opacity/clip-path. It is clipped
      // to zero area, paints nothing and cannot be clicked -- yet it reads as a
      // frozen scroll-reveal whenever its fixed width happens to exceed 25% of
      // the viewport (the isContentSized threshold), so it gets reported on
      // phones and nowhere else.
      //
