# intuitcomintuitintelligence-2

A pixel-faithful Next.js + TypeScript replica of [https://www.intuit.com/intuit-intelligence/](https://www.intuit.com/intuit-intelligence/).
...
npm run dev          # http://localhost:3309
```
...
### Known capture limitation: the mobile nav's burger button

Intuit's global nav is a client-side component that renders **different markup
per viewport width**, and the capture ran at desktop width. So the generated
markup contains the desktop variant only: the 30px brands bar, the
`S01MegaNav-is-desktop-abf7ba5` class and the horizontal links list.

`app/lib/intuitBrandsNav.ts` reproduces the width-driven swap the live component
performs, using live's own class names (measured: the switch is at **1120px** —
1119px serves the hamburger variant, 1120px the desktop one). Below 1120px it
hides the brands bar via `MGlobalNavigationBrands-hide-b0fb1fe`, swaps in
`S01MegaNav-is-hamburger-menu-c6ab1a2`, hides the desktop-only links/contact
wrappers that live omits from the DOM, and drives the fixed mega-nav's offset
from the bar's *measured* height (so it becomes 0 exactly when the bar is
hidden). That removes the overflowing desktop links and the 30px downward shift
of every section below the nav.

What is **not** reproduced: the burger button itself
(`S01MegaNav-overlay-navigation-button-*`, `AButton-icon-button-8d47c3b`) and
its overlay menu. That markup is only emitted below 1120px, so a desktop capture
never received it — the CSS is present in the captured stylesheets but there is
no element to style. It is deliberately left absent rather than hand-written,
because inventing UI the capture never saw is no longer replication. To close
this gap properly, re-run `npm run capture` at a mobile width and merge the two
nav variants.

Residual visual diff is therefore dominated by three *expected* sources rather
than layout bugs: the live cookie-consent banner (absent locally, and roughly a
third of a 667px phone viewport, which is why the smallest viewports score
worst), the letter-trail animation being at a different phase when each
screenshot is taken, and this missing burger icon. Footer geometry was verified
identical to live at 375px.
