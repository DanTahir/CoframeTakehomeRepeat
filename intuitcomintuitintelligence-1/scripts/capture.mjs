...
function readHead() {
  ...
  // Stylesheets AND <style> blocks can also appear in <body>, and they cascade
  // after everything in <head>. Real sites do this routinely (intuit.com links
  // its page-specific sheet just before its scripts), so both kinds are
  // recorded here in document order for codegen to rebuild the cascade from.
  //
  // Recording only <style> silently dropped body-linked sheets: the JSX walker
  // kept the <link> tag, so the file still loaded and the page looked roughly
  // right, but codegen never *emitted* the sheet -- meaning its url() refs were
  // never rewritten and the replica re-fetched webfonts/images from the origin
  // CDN at runtime, breaking the zero-remote-requests guarantee.
  const bodyEntries = [];
  for (const node of Array.from(document.body.querySelectorAll('style, link'))) {
    if (node.tagName.toLowerCase() === 'style') {
      bodyEntries.push({ kind: 'style', css: node.textContent ?? '' });
      continue;
    }
  ...
  // Kept for captures/consumers predating bodyEntries.
  const bodyStyles = bodyEntries.filter((e) => e.kind === 'style').map((e) => e.css);
