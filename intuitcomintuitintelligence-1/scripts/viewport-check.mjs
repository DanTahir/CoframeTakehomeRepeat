...
async function findHiddenContent(page, scrollSettleMs, thorough) {
  ...
  return page.evaluate(
    async ({ quickMs: quick, confirmMs: confirm, thorough: strict }) => {
      ...
      // A COLLAPSED DISCLOSURE PANEL is deliberately-hidden UI as well, but
      // plenty of components hide one with opacity/clip-path instead of display
      // or visibility, where the structural walk above cannot see it. This
      // page's country selector is the case in point:
      //   .MCountrySelector-options { opacity: 0;
      //                               clip-path: inset(0 0 100% 0);
      //                               pointer-events: none }
      // and only `.MCountrySelector-show` restores opacity/clip-path. It is
      // clipped to zero area, paints nothing and cannot be clicked -- yet it
      // read as a frozen scroll-reveal purely because 182px happens to exceed
      // 25% of a phone viewport (the isContentSized threshold), which is why it
      // was reported below 768px and nowhere else.
      //
      // The authoritative signal is ARIA, not styling: a control that owns the
      // panel via aria-controls while reporting aria-expanded="false". That is
      // the same class of evidence as the role/dialog checks above.
      // Recomputed per call rather than snapshotted, so it reflects the state at
      // sample time; it stays cheap because only on-screen, content-sized,
      // opacity~0 candidates ever reach this point.
      const ownedByCollapsedControl = (el) => {
        const collapsedIds = new Set();
        for (const ctrl of Array.from(
          document.querySelectorAll('[aria-expanded="false"][aria-controls]'),
        )) {
          for (const id of (ctrl.getAttribute('aria-controls') || '').split(/\s+/)) {
        ...
        for (let n = el; n && n.nodeType === 1; n = n.parentElement) {
          if (n.id && collapsedIds.has(n.id)) return true;
        }
        return false;
      };
      ...
      const sample = () => {
        ...
        for (const el of Array.from(document.querySelectorAll('body *'))) {
          ...
          if (ownedByCollapsedControl(el)) continue;
