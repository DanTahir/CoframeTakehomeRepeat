...
import { initBrandsNav } from '../app/lib/intuitBrandsNav';
import {
  createAccordion,
  createCounters,
  createLazyImages,
...
describe('brandsNav', () => {
  // Mirrors the captured nav: the brand strip sits in flow above a position:fixed nav.
  const markup = `
    <div id="brands-navigation" class="MGlobalNavigationBrands-top-navigation-ce29b8d"></div>
    <nav id="main-navigation" class="S01MegaNav-global-navigation-wrapper-b5210ed navbar">
      <div id="nav-inner"
           class="S01MegaNav-global-navigation-f05a4e9 S01MegaNav-top-fixed-83f40c5"></div>
    </nav>`;

  const HIDE_CLASS = 'MGlobalNavigationBrands-hide-b0fb1fe';

  const setWidth = (width: number) =>
    Object.defineProperty(window, 'innerWidth', { value: width, configurable: true });

  // 1119 is a boundary guard: live is measurably still in its narrow state there.
  it('hides the strip and pins the nav to the top below 1120', () => {
    document.body.innerHTML = markup;
    setWidth(1119);
    const strip = document.getElementById('brands-navigation')!;
    const nav = document.getElementById('nav-inner')!;

    const teardown = initBrandsNav(document);

    expect(strip.classList.contains(HIDE_CLASS)).toBe(true);
    expect(nav.style.top).toBe('0px');
    // Live carries the inline height at every width, shown or hidden.
    expect(strip.style.height).toBe('30px');

    if (typeof teardown === 'function') teardown();
  });

  it('shows the strip and offsets the nav by its height from 1120 up', () => {
    document.body.innerHTML = markup;
    setWidth(1120);
    const strip = document.getElementById('brands-navigation')!;
    const nav = document.getElementById('nav-inner')!;
    ...
    expect(strip.classList.contains(HIDE_CLASS)).toBe(false);
    expect(nav.style.top).toBe('30px');

    if (typeof teardown === 'function') teardown();
  });

  it('re-applies when a resize crosses the breakpoint', () => {
    document.body.innerHTML = markup;
    setWidth(1440);
    const strip = document.getElementById('brands-navigation')!;
    const nav = document.getElementById('nav-inner')!;
    ...
    setWidth(900);
    // rAF is synchronous in setup.ts, so the rafThrottled handler runs immediately.
  ...
  it('no-ops without a brand strip', () => {
    document.body.innerHTML = '<nav id="main-navigation"></nav>';
    expect(initBrandsNav(document)).toBeUndefined();
  });
});
