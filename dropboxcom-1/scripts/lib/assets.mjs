...
export function refsFromHtml(html, baseUrl, isBlocked) {
  ...
  // drops the legacy two-token form `rel="shortcut icon"` — which is exactly
  // what dropbox.com emits for its favicon. The icon then never enters the
  // asset map, codegen drops it from metadata for having no local copy, and the
  // browser falls back to probing /favicon.ico at the origin root. That probe
  // comes from the BROWSER process, so Playwright's request events never see it
  // and it surfaces only as an unattributable console 404 — which fails the
